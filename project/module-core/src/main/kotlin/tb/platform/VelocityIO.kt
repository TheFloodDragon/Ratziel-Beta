...
class VelocityIO : PlatformIO {
    ...
    override fun releaseResourceFile(resource: String, target: String, replace: Boolean): File {
        ...
        newFile(file).writeBytes(
            javaClass.classLoader.getResourceAsStream(resource)?.readBytes() ?: error("resource not found: $resource")
        )
        return file
    }

    override fun getJarFile(): File {
        return VelocityPlugin.getPluginInstance()?.nativeJarFile()
            ?: File(VelocityPlugin::class.java.protectionDomain.codeSource.location.toURI().path)
    }

    override fun getDataFolder(): File {
        return VelocityPlugin.getPluginInstance()?.nativeDataFolder()
            ?: VelocityPlugin.getInstance().configDirectory.toFile()
    }
