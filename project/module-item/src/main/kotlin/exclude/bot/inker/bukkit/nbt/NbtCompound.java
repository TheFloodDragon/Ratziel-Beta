...
import exclude.bot.inker.bukkit.nbt.internal.loader.DelegateAbstractMap;
import exclude.bot.inker.bukkit.nbt.internal.loader.LazyLoadEntrySet;
import exclude.bot.inker.bukkit.nbt.internal.loader.StringUtils;
...
public class NbtCompound extends Nbt<RefNbtTagCompound> implements NbtComponentLike {
  ...
  public Nbt<?> get(String key) {
    return fromNms(delegate.get(key));
  }

  @Override
  public Nbt<?> get(Object key) {
    return (key instanceof String) ? fromNms(delegate.get((String) key)) : null;
  }
  ...
  public Nbt<?> put(String key, Nbt<?> value) {
    if (SET_RETURN_SUPPORT) {
      return fromNms(delegate.set1(key, value.delegate));
    } else {
      Nbt<?> previous = fromNms(delegate.get(key));
