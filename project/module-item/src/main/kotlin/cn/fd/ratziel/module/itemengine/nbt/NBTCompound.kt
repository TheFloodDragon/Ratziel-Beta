...
open class NBTCompound(rawData: Any) : NBTData(rawData, NBTDataType.COMPOUND) {
     ...
     * @param node 节点
     */
    open operator fun get(node: String): NBTData? = supportList(node) { pair ->
        getGenerally(pair.first).let { pair.second?.let { i -> (it as NBTList)[i] } ?: it }
    }


    open fun getGenerally(node: String): NBTData? =
     ...
     * @param node 节点
     */
    open fun getDeep(node: String): NBTData? =
     ...
     * @param value NBT数据
     */
    open fun put(node: String, value: NBTData?) = supportList(node) { pair ->
        putGenerally(
    ...
    open fun putGenerally(node: String, value: NBTData?) {
     ...
     * 深度写入
     */
    open fun putDeep(node: String, value: NBTData?) {
     ...
     * @param node 节点
     */
    open fun remove(node: String) = supportList(node) { pair ->
    ...
    open fun removeGenerally(node: String) {
     ...
     * 深度删除，以 "." 作为分层符
     */
    open fun removeDeep(node: String) {
        if (node == APEX_NODE_SIGN) return
     ...
     * 针对"深度方法"的重复代码做出的优化
     */
    open fun getDeepWith(node: String, create: Boolean, action: (NBTCompound) -> Any?): NBTData? {
        // 分割节点 (丢弃最后一层)
     ...
     * 批量设置数据
     */
    open fun putAll(vararg dataPair: Pair<String, NBTData?>, deep: Boolean = false) = this.apply {
     ...
     */
    open fun <T : NBTData> editShallow(
        node: String,
        default: T,
        function: Consumer<T>,
    ...
    open fun editShallow(node: String, function: Consumer<NBTTag>): NBTTag = editShallow(node, NBTTag(), function)

    /**
     * 合并复合标签
     * @param replace 是否替换原有的标签
     */
    open fun merge(target: NBTCompound, replace: Boolean = true): NBTCompound = this.also { source ->
    ...
    open fun clone() = this.apply {
        this.data = if (isTiNBT()) (data as TiNBTTag).clone() else NMSMethods.cloneMethod.invoke(data)!!
    }
     ...
     */
    @JvmOverloads
    open fun toMapDeep(source: Map<String, Any>? = toMapUnsafe()): Map<String, NBTData> = buildMap {
     ...
     */
    open fun toMapShallow(source: Map<String, Any>? = toMapUnsafe()): Map<String, NBTData> = buildMap {
     ...
     */
    open fun toMapUnsafe(): Map<String, Any>? =
        (if (isTiNBT()) data else NMSMethods.mapField.get(data)) as? Map<String, Any>

    /**
     * Kotlin操作符
     */
    open operator fun set(node: String, value: NBTData?) = put(node, value)
    open operator fun minus(node: String) = remove(node)
